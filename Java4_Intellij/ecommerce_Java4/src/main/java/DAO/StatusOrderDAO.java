...
public class StatusOrderDAO {
    ...
    public List<FlowStatus> historyOrder(int id) {
        List<FlowStatus> list = new ArrayList<>();
        String sql = "SELECT orders.id,p.id,imageProduct,nameProduct,size,color,o_quantity,price,price * orders.o_quantity as total, statusState\n" +
                ...
                "where u_id = ? and statusState = 2 or statusState = 3 or statusState = 4";
        try {
            Connection con = BaseService.getConnection();
            PreparedStatement ps = con.prepareStatement(sql);
            ps.setInt(1, id);
